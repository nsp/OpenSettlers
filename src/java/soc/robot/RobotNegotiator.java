...
public class RobotNegotiator
{
    ...
    protected PossiblePiece[] targetPieces;
     ...
     */
    public RobotNegotiator(RobotBrain br)
    {
        brain = br;
        ...
        isSellingResource = new boolean[game.maxPlayers][ResourceConstants.MAXPLUSONE];
        resetIsSelling();

        wantsAnotherOffer = new boolean[game.maxPlayers][ResourceConstants.MAXPLUSONE];
        resetWantsAnotherOffer();

        offersMade = new Vector();

        targetPieces = new PossiblePiece[game.maxPlayers];
        resetTargetPieces();
    }
     ...
     */
    public void setTargetPiece(int pn, PossiblePiece piece)
    {
     ...
     */
    public void addToOffersMade(TradeOffer offer)
    {
        if (offer != null)
        {
     ...
     */
    public TradeOffer makeOffer(PossiblePiece targetPiece)
    {
        ...
        ResourceSet targetResources = null;

        switch (targetPiece.getType())
        {
        case PossiblePiece.CARD:
            targetResources = Game.CARD_SET;

            break;

        case PossiblePiece.ROAD:
            targetResources = Game.ROAD_SET;

            break;

        case PossiblePiece.SETTLEMENT:
            targetResources = Game.SETTLEMENT_SET;

            break;

        case PossiblePiece.CITY:
            targetResources = Game.CITY_SET;
        ...
        if (ourResources.getAmount(ResourceConstants.UNKNOWN) > 0)
        {
        ...
        TradeOffer batna = getOfferToBank(targetResources);
        D.ebugPrintln("*** BATNA = " + batna);

        BuildingSpeedEstimate estimate = new BuildingSpeedEstimate(ourPlayerData.getNumbers());

        ResourceSet giveResourceSet = new ResourceSet();
        ResourceSet getResourceSet = new ResourceSet();
        ...
        ///
        boolean[] someoneIsSellingResource = new boolean[ResourceConstants.MAXPLUSONE];
        ...
        if (getRsrcIdx >= 0)
        {
            ...
            ResourceSet leftovers = ourResources.copy();
     ...
     */
    protected TradeOffer makeOfferAux(ResourceSet giveResourceSet, ResourceSet getResourceSet, int neededResource)
    {
        ...
        {
            TradeOffer pastOffer = (TradeOffer) offersMadeIter.next();
        ...
        if (!match)
        {
            ...
            for (int i = 0; i < game.maxPlayers; i++)
            {
                if (i != opn)
                {
                    TradeOffer outsideOffer = game.getPlayer(i).getCurrentOffer();
        ...
        if (!match)
        {
            ...
            if (game.getCurrentPlayerNumber() == opn)
            {
                ...
                for (int i = 0; i < game.maxPlayers; i++)
                {
                    ...
                    {
                        PlayerTracker tracker = (PlayerTracker) playerTrackers.get(new Integer(i));
            ...
            else
            {
                ...
                if (isSellingResource[curpn][neededResource] && (game.getPlayer(curpn).getResources().getTotal() >= getResourceSet.getTotal()))
                {
                    ...
                    PlayerTracker tracker = (PlayerTracker) playerTrackers.get(new Integer(curpn));
            ...
            if (numOfferedTo > 0)
            {
                ...
                ///  the offer
                ///
                offer = new TradeOffer(game.getName(), ourPlayerData.getPlayerNumber(), offeredTo, giveResourceSet, getResourceSet);
     ...
     * @param estimate           a BuildingSpeedEstimate for our player
     */
    protected int getETAToTargetResources(Player player, ResourceSet targetResources, ResourceSet giveSet, ResourceSet getSet, BuildingSpeedEstimate estimate)
    {
        ResourceSet ourResourcesCopy = player.getResources().copy();
        ...
        try
        {
            ResSetBuildTimePair offerBuildingTimePair = estimate.calculateRollsFast(ourResourcesCopy, targetResources, 1000, player.getPortFlags());
     ...
     */
    public int considerOffer2(TradeOffer offer, int receiverNum)
    {
        ///
        ...
        Player receiverPlayerData = game.getPlayer(receiverNum);
        ResourceSet receiverResources = receiverPlayerData.getResources();

        ResourceSet rsrcsOut = offer.getGetSet();
        ResourceSet rsrcsIn = offer.getGiveSet();

        //
        // if the receiver doesn't have what's asked for, they'll reject
        //
        if ((receiverResources.getAmount(ResourceConstants.UNKNOWN) == 0) && (!receiverResources.contains(rsrcsOut)))
        {
            return response;
        }
        ...
        PossiblePiece receiverTargetPiece = targetPieces[receiverNum];

        D.ebugPrintln("targetPieces[" + receiverNum + "] = " + receiverTargetPiece);

        PlayerTracker receiverPlayerTracker = (PlayerTracker) playerTrackers.get(new Integer(receiverNum));
        ...
        PlayerTracker senderPlayerTracker = (PlayerTracker) playerTrackers.get(new Integer(senderNum));
        ...
        if (receiverTargetPiece == null)
        {
            ...
            simulator = new RobotDM(brain.getRobotParameters(), playerTrackers, receiverPlayerTracker, receiverPlayerData, receiverBuildingPlan);
            ...
            receiverTargetPiece = (PossiblePiece) receiverBuildingPlan.peek();
            targetPieces[receiverNum] = receiverTargetPiece;
        }

        D.ebugPrintln("receiverTargetPiece = " + receiverTargetPiece);

        PossiblePiece senderTargetPiece = targetPieces[senderNum];

        D.ebugPrintln("targetPieces[" + senderNum + "] = " + senderTargetPiece);

        Player senderPlayerData = game.getPlayer(senderNum);

        if (senderTargetPiece == null)
        {
            Stack senderBuildingPlan = new Stack();
            simulator = new RobotDM(brain.getRobotParameters(), playerTrackers, senderPlayerTracker, senderPlayerData, senderBuildingPlan);
            ...
            senderTargetPiece = (PossiblePiece) senderBuildingPlan.peek();
        ...
        if (senderWGETA > WIN_GAME_CUTOFF)
        {
            ...
            if ((receiverTargetPiece.getType() == PossiblePiece.SETTLEMENT) || (receiverTargetPiece.getType() == PossiblePiece.ROAD))
            {
                Enumeration threatsEnum = receiverTargetPiece.getThreats().elements();

                while (threatsEnum.hasMoreElements())
                {
                    PossiblePiece threat = (PossiblePiece) threatsEnum.nextElement();
                ...
                else if (receiverTargetPiece.getType() == PossiblePiece.SETTLEMENT)
                {
                    Enumeration conflictsEnum = ((PossibleSettlement) receiverTargetPiece).getConflicts().elements();

                    while (conflictsEnum.hasMoreElements())
                    {
                        PossibleSettlement conflict = (PossibleSettlement) conflictsEnum.nextElement();

                        if ((senderTargetPiece.getType() == PossiblePiece.SETTLEMENT) && (conflict.getCoordinates() == senderTargetPiece.getCoordinates()))
                        {
                            inARace = true;
            ...
            if (!inARace)
            {
                ...
                BuildingSpeedEstimate estimate = new BuildingSpeedEstimate(receiverPlayerData.getNumbers());

                TradeOffer receiverBatna = getOfferToBank(targetResources);
                D.ebugPrintln("*** receiverBatna = " + receiverBatna);

                int batnaBuildingTime = getETAToTargetResources(receiverPlayerData, targetResources, ResourceSet.EMPTY_SET, ResourceSet.EMPTY_SET, estimate);
    ...
    /*
       public int considerOffer(TradeOffer offer) {
       ...
       ResourceSet theirResources = offeringPlayer.getResources();
       ResourceSet ourResources = ourPlayerData.getResources();
       ...
       simulator = new RobotDM(brain.getRobotParameters(),
       playerTrackers,
       ourPlayerTracker,
       ourPlayerData,
       ourBuildingPlan);
       ...
       PossiblePiece targetPiece = (PossiblePiece)ourBuildingPlan.peek();
       ...
       PlayerTracker theirPlayerTracker = (PlayerTracker)playerTrackers.get(new Integer(offer.getFrom()));
       ...
       theirBuildingPlan);
       simulator.planStuff();
       theirOriginalPiece = (PossiblePiece)theirBuildingPlan.pop();
       ...
       simulator.planStuff();
       PossiblePiece ourNewTargetPiece = (PossiblePiece)ourBuildingPlanAfter.pop();
       ...
       theirBuildingPlan);
       simulator.planStuff();
       PossiblePiece theirNewTargetPiece = (PossiblePiece)theirBuildingPlan.pop();
       ...
       //
       boolean inARace = false;
       if ((ourOriginalPieceType == PossiblePiece.SETTLEMENT) ||
       (ourOriginalPieceType == PossiblePiece.ROAD)) {
       ...
       } else if (ourOriginalPieceType == PossiblePiece.SETTLEMENT) {
       Enumeration conflictsEnum = ((PossibleSettlement)ourOriginalPiece).getConflicts().elements();
       while (conflictsEnum.hasMoreElements()) {
       PossibleSettlement conflict = (PossibleSettlement)conflictsEnum.nextElement();
     ...
     */
    public TradeOffer makeCounterOffer(TradeOffer originalOffer)
    {
        D.ebugPrintln("***** MAKE COUNTER OFFER *****");

        TradeOffer counterOffer = null;

        PossiblePiece targetPiece = targetPieces[ourPlayerData.getPlayerNumber()];
        ...
        if (targetPiece == null)
        {
            ...
            if (ourBuildingPlan.empty())
            {
                RobotDM simulator;
                D.ebugPrintln("**** our building plan is empty ****");
                simulator = new RobotDM(brain.getRobotParameters(), playerTrackers, ourPlayerTracker, ourPlayerData, ourBuildingPlan);
                simulator.planStuff(strategyType);
            }
            ...
            targetPiece = (PossiblePiece) ourBuildingPlan.peek();
     ...
     */
    public TradeOffer getOfferToBank(ResourceSet targetResources, ResourceSet ourResources)
    {
        TradeOffer bankTrade = null;
        ...
        while (giveRsrcIdx < notNeededRsrcCount)
        {
            ...
            if (ourResources.getAmount(notNeededRsrc[giveRsrcIdx]) >= tradeRatio)
            {
                ...
                bankTrade = new TradeOffer(game.getName(), ourPlayerData.getPlayerNumber(), to, give, get);
     ...
     * @param targetResources  what resources we want
     */
    public TradeOffer getOfferToBank(ResourceSet targetResources)
    {
