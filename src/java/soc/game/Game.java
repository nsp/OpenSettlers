 ...
 *<P>
 * The model in this client/server game is: The Game at server contains the game's
 * complete state information, and game logic advances there.
 * Each client's local Game contains only partial state (for instance, other
 ...
 * {@link soc.message.GameState} and {@link soc.message.SetPlayedDevCard}.
 *<P>
 * For the board <b>coordinate system and terms</b> (hex, node, edge), see the
 * {@link Board} class javadoc.
 *<P>
 * {@link #putPiece(PlayingPiece)} and other game-action methods update gameState.
 *
...
public class Game implements Serializable, Cloneable
{
     ...
     *<UL>
     * <LI> {@link soc.client.BoardPanel#updateMode()}
     * <LI> {@link soc.client.BuildingPanel#updateButtonStatus()}
     * <LI> {@link soc.client.PlayerInterface#updateAtGameState()}
     * <LI> {@link #putPiece(PlayingPiece)}
     * <LI> {@link #advanceTurnStateAfterPutPiece()}
     * <LI> {@link #forceEndTurn()}
     * <LI> {@link soc.robot.RobotBrain#run()}
     * <LI> {@link soc.server.SOCServer#sendGameState(Game)}
     *</UL>
     ...
     *<UL>
     * <LI> BoardPanel.BoardPopupMenu.showBuild, showCancelBuild
     * <LI> BoardPanel.drawBoard
     * <LI> HandPanel.addPlayer, began, removePlayer, updateAtTurn, updateValue
     * <LI> Game.addPlayer
     * <LI> SOCServer.handleSTARTGAME, leaveGame, sitDown, handleCANCELBUILDREQUEST, handlePUTPIECE
     * <LI> PlayerClient.handleCANCELBUILDREQUEST, DisplaylessPlayerClient.handleCANCELBUILDREQUEST
     *</UL>
     */
    ...
    public static final int SETOPTIONS_INCL = 3; // Future use: Game owner setting options, but anyone can connect
        ...
        // because they are set before the Game is created.
     ...
     * @see #SETTLEMENT_SET
     */
    public static final ResourceSet EMPTY_RESOURCES = new ResourceSet();

    /**
     * the set of resources a player needs to build a settlement
     * @see PlayingPiece#getResourcesToBuild(int)
     */
    public static final ResourceSet SETTLEMENT_SET = new ResourceSet(1, 0, 1, 1, 1, 0);

    /**
     * the set of resources a player needs to build a road
     * @see PlayingPiece#getResourcesToBuild(int)
     */
    public static final ResourceSet ROAD_SET = new ResourceSet(1, 0, 0, 0, 1, 0);

    /**
     * the set of resources a player needs to build a city
     * @see PlayingPiece#getResourcesToBuild(int)
     */
    public static final ResourceSet CITY_SET = new ResourceSet(0, 3, 0, 2, 0, 0);

    /**
     * the set of resources a player needs to buy a development card
     * @see PlayingPiece#getResourcesToBuild(int)
     */
    public static final ResourceSet CARD_SET = new ResourceSet(0, 1, 1, 1, 0, 0);
     ...
     * or -1 if unknown.
     * Calculated by {@link GameOption#optionsMinimumVersion(Hashtable)}.
     ...
     * @since 1.1.07
     */
    public transient GameBoardReset boardResetOngoingInfo;
    ...
    /**
     * the game options ({@link GameOption}), or null
     * @since 1.1.07
     */
     ...
     *           control characters, {@link Message#sep_char}, or {@link Message#sep2_char}.
     *           This is enforced by calling {@link Message#isSingleLineAndSafe(String)}.
     * @param op if game has options, hashtable of {@link GameOption}; otherwise null.
     ...
     *           {@link GameOption#optionsMinimumVersion(Hashtable)}.
     * @throws IllegalArgumentException if op contains unknown options, or any
     *             object class besides {@link GameOption}
     * @since 1.1.07
     */
    public Game(String n, Hashtable op)
     ...
     *           {@link Message#isSingleLineAndSafe(String)}. This check was added in 1.1.07.
     */
    public Game(String n, boolean a)
        throws IllegalArgumentException
    {
        this(n, a, null);
    }
     ...
     *           {@link GameOption#optionsMinimumVersion(Hashtable)}.
     * @throws IllegalArgumentException if op contains unknown options, or any
     *             object class besides {@link GameOption}, or if game name
     *             fails {@link Message#isSingleLineAndSafe(String)}.
     * @since 1.1.07
     */
    public Game(String n, boolean a, Hashtable op)
        throws IllegalArgumentException
    {
        ...
        if (op != null)
        {
           GameOption maxpl = (GameOption) op.get("PL");
        ...
        if (op == null)
        {
            ...
            if (! GameOption.adjustOptionsToKnown(op, null))
                throw new IllegalArgumentException("op: unknown option");

            // the adjust method will also throw IllegalArg if a non-GameOption
            // object is found within opts.

            clientVersionMinRequired = GameOption.optionsMinimumVersion(op);
        }
     ...
     * add a new player
     *
     * @param name  the player's name; must pass {@link Message#isSingleLineAndSafe(String)}.
     ...
     * @throws IllegalArgumentException if name fails {@link Message#isSingleLineAndSafe(String)}.
     *           This exception was added in 1.1.07.
     */
    public void addPlayer(final String name, final int pn)
        throws IllegalStateException, IllegalArgumentException
    {
        if (! Message.isSingleLineAndSafe(name))
     ...
     */
    public Player getPlayer(int pn)
    {
        return players[pn];
    }
     ...
     */
    public Player getPlayer(String nn)
    {
        if (nn != null)
        {
            for (int i = 0; i < maxPlayers; i++)
    ...
    /**
     * @return this game's options ({@link GameOption}), or null
     * @since 1.1.07
     ...
     * @param optKey Name of a {@link GameOption}
     ...
     * @param optKey Name of a {@link GameOption} of type {@link GameOption#OTYPE_BOOL OTYPE_BOOL},
    ...
    /**
     * Is this boolean-valued game option currently set to true?
     * @param opts A hashtable of {@link GameOption}, or null
     * @param optKey Name of a {@link GameOption} of type {@link GameOption#OTYPE_BOOL OTYPE_BOOL},
     *               {@link GameOption#OTYPE_INTBOOL OTYPE_INTBOOL}
     *               or {@link GameOption#OTYPE_ENUMBOOL OTYPE_ENUMBOOL}
     * @return True if option is set, false if not set or not defined in this game's options
    ...
    public static boolean isGameOptionSet(Hashtable opts, final String optKey)
    {
        ...
        if (opts == null)
            return false;
        GameOption op = (GameOption) opts.get(optKey);
        if (op == null)
            return false;
     ...
     * @param optKey A {@link GameOption} of type {@link GameOption#OTYPE_INT OTYPE_INT},
     *               {@link GameOption#OTYPE_INTBOOL OTYPE_INTBOOL},
     *               {@link GameOption#OTYPE_ENUM OTYPE_ENUM}
     *               or {@link GameOption#OTYPE_ENUMBOOL OTYPE_ENUMBOOL}
     * @return Option's current {@link GameOption#getIntValue() intValue},
     ...
     * @param optKey A {@link GameOption} of type
     *               {@link GameOption#OTYPE_STR OTYPE_STR}
     *               or {@link GameOption#OTYPE_STRHIDE OTYPE_STRHIDE}
     * @return Option's current {@link GameOption#getStringValue() getStringValue}
     ...
     * option version data is hardcoded in {@link GameOption#getMinVersion()},
     ...
     */
    protected void setBoard(Board gb)
    {
        board = gb;
    }
     ...
     */
    protected void setPlayer(int pn, Player pl)
    {
        players[pn] = pl;
    }
     ...
     * @param pl  the player
     */
    public void setPlayerWithLargestArmy(Player pl)
    {
        if (pl == null)
        {
            playerWithLargestArmy = -1;
     ...
     */
    public Player getPlayerWithWin()
    {
        if (playerWithWin != -1)
        {
     ...
     */
    public String gameOverMessageToPlayer(Player pl)
        throws IllegalStateException
    {
        if (gameState != OVER)
            throw new IllegalStateException("This game is not over yet");
        String msg;
        Player wn = getPlayerWithWin();
     ...
     * the player some resources, and call their {@link Player#clearPotentialSettlements()}.
     *<P>
     * Calls {@link Board#putPiece(PlayingPiece)} and {@link Player#putPiece(PlayingPiece)}.
    ...
    public void putPiece(PlayingPiece pp)
    {
         ...
         */
        if (pp.getType() == PlayingPiece.CITY)
        {
            Settlement se = new Settlement(pp.getPlayer(), pp.getCoordinates(), board);
         ...
         */
        if ((gameState == START2A) && (pp.getType() == PlayingPiece.SETTLEMENT))
        {
            ...
            while (hexes.hasMoreElements())
            {
                ...
                {
                case Board.CLAY_HEX:
                    resources.add(1, ResourceConstants.CLAY);

                    break;

                case Board.ORE_HEX:
                    resources.add(1, ResourceConstants.ORE);

                    break;

                case Board.SHEEP_HEX:
                    resources.add(1, ResourceConstants.SHEEP);

                    break;

                case Board.WHEAT_HEX:
                    resources.add(1, ResourceConstants.WHEAT);

                    break;

                case Board.WOOD_HEX:
                    resources.add(1, ResourceConstants.WOOD);
        ...
        if ((gameState == START2B) && (pp.getType() == PlayingPiece.ROAD))
        {
        ...
        if (pp.getType() != PlayingPiece.CITY)
        {
            if (pp.getType() == PlayingPiece.ROAD)
            {
                /**
            ...
            else
            {
                ...
                while (adjEdgeEnum.hasMoreElements())
                {
                    ...
                    {
                        Road road = (Road) allRoadsEnum.nextElement();
     ...
     * @param pp the piece to put on the board
     */
    public void putTempPiece(PlayingPiece pp)
    {
         ...
         */
        oldPlayerWithLongestRoad.push(new OldLRStats(this));
     ...
     * @param pp the piece to put on the board
     */
    protected void undoPutPieceCommon(PlayingPiece pp)
    {
     ...
     * @param pp the piece to put on the board
     *
     * @see #undoPutInitSettlement(PlayingPiece)
     */
    public void undoPutTempPiece(PlayingPiece pp)
    {
        ...
        // update which player has longest road
        //
        OldLRStats oldLRStats = (OldLRStats) oldPlayerWithLongestRoad.pop();
    ...
    public void startGame()
    {
        ...
        for (i = 0; i < 14; i++)
        {
            devCardDeck[i] = DevCardConstants.KNIGHT;
        }

        for (i = 14; i < 16; i++)
        {
            devCardDeck[i] = DevCardConstants.ROADS;
        }

        for (i = 16; i < 18; i++)
        {
            devCardDeck[i] = DevCardConstants.MONO;
        }

        for (i = 18; i < 20; i++)
        {
            devCardDeck[i] = DevCardConstants.DISC;
        }

        devCardDeck[20] = DevCardConstants.CAP;
        devCardDeck[21] = DevCardConstants.LIB;
        devCardDeck[22] = DevCardConstants.UNIV;
        devCardDeck[23] = DevCardConstants.TEMP;
        devCardDeck[24] = DevCardConstants.TOW;
        ...
        if (maxPlayers > 4)
        {
            ...
            devCardDeck[31] = DevCardConstants.ROADS;
            devCardDeck[32] = DevCardConstants.MONO;
            devCardDeck[33] = DevCardConstants.DISC;
        }
     ...
     * Does not clear any player's {@link Player#hasAskedSpecialBuild()} flag.
     *<P>
     ...
     * {@link ForceEndTurnResult#FORCE_ENDTURN_NONE FORCE_ENDTURN_NONE}.
     ...
     * <UL>
     * <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_RSRC_DISCARD_WAIT}
     ...
     *         as yet unchanged.
     * <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_ADV}
     ...
     * <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_ADVBACK}
     *       - During initial placement, have skipped placement of
     ...
     *       {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_TURN}.
     * </UL>
     *
     * @return Type of action performed, one of these values:
     *     <UL>
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_NONE}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_ADV}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_ADVBACK}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_TURN}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_RSRC_RET_UNPLACE}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_UNPLACE_ROBBER}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_RSRC_DISCARD}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_RSRC_DISCARD_WAIT}
     *     <LI> {@link ForceEndTurnResult#FORCE_ENDTURN_LOST_CHOICE}
     *     </UL>
     ...
     * @see #endTurn()
     */
    public ForceEndTurnResult forceEndTurn()
        throws IllegalStateException
    {
        ...
        case PLAY:
            gameState = PLAY1;
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_NONE);
        ...
        case PLACING_ROAD:
            ...
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_RSRC_RET_UNPLACE, ROAD_SET);

        case PLACING_SETTLEMENT:
            cancelBuildSettlement(currentPlayerNumber);
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_RSRC_RET_UNPLACE, SETTLEMENT_SET);

        case PLACING_CITY:
            cancelBuildCity(currentPlayerNumber);
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_RSRC_RET_UNPLACE, CITY_SET);
        ...
        case PLACING_ROBBER:
            {
                ...
                {
                    placingRobberForKnightCard = false;
                    players[currentPlayerNumber].getDevCards().add(1, DevCardSet.OLD, DevCardConstants.KNIGHT);
                }
                return new ForceEndTurnResult
                    (ForceEndTurnResult.FORCE_ENDTURN_UNPLACE_ROBBER,
                     isFromDevCard ? DevCardConstants.KNIGHT : -1);
            }
        ...
        case PLACING_FREE_ROAD2:
            gameState = PLAY1;
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_RSRC_RET_UNPLACE);
        ...
        case WAITING_FOR_CHOICE:
            gameState = PLAY1;
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_LOST_CHOICE);

        case WAITING_FOR_DISCOVERY:
            gameState = PLAY1;
            players[currentPlayerNumber].getDevCards().add(1, DevCardSet.OLD, DevCardConstants.DISC);
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_LOST_CHOICE,
                 DevCardConstants.DISC);

        case WAITING_FOR_MONOPOLY:
            gameState = PLAY1;
            players[currentPlayerNumber].getDevCards().add(1, DevCardSet.OLD, DevCardConstants.MONO);
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_LOST_CHOICE,
     ...
     *         {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_ADV},
     *         {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_ADVBACK},
     *         or {@link ForceEndTurnResult#FORCE_ENDTURN_SKIP_START_TURN}.
     */
    private ForceEndTurnResult forceEndTurnStartState(boolean advTurnForward)
    {
        ...
        if (cpn == currentPlayerNumber)
        {
            ...
            if (advTurnForward)
            {
                ...
                cancelResType = ForceEndTurnResult.FORCE_ENDTURN_SKIP_START_ADVBACK;
            } else {
                // Was second placement; begin normal gameplay.
                // Set resType to tell caller to call endTurn().
                gameState = PLAY1;
                cancelResType = ForceEndTurnResult.FORCE_ENDTURN_SKIP_START_TURN;
            }
        } else {
            // OK, player has changed.  This means advanceTurnStateAfterPutPiece()
            // has also cleared the forcingEndTurn flag.
            if (advTurnForward)
                cancelResType = ForceEndTurnResult.FORCE_ENDTURN_SKIP_START_ADV;
            else
                cancelResType = ForceEndTurnResult.FORCE_ENDTURN_SKIP_START_ADVBACK;
        }
        ...
        return new ForceEndTurnResult(cancelResType, updateFirstPlayer, updateLastPlayer);
    }

    /**
     * Randomly discard from this player's hand, by calling {@link #discard(int, ResourceSet)}.
     ...
     *         Type will be {@link ForceEndTurnResult#FORCE_ENDTURN_RSRC_DISCARD}
     *         or {@link ForceEndTurnResult#FORCE_ENDTURN_RSRC_DISCARD_WAIT}.
     */
    private ForceEndTurnResult forceEndTurnChkDiscards(int pn)
    {
        // select random cards, and discard
        ResourceSet discards = new ResourceSet();
        {
            ResourceSet hand = players[pn].getResources(); 
        ...
        {
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_RSRC_DISCARD_WAIT, discards, true);
        } else {
            // gameState == PLAY1 - was set in discard()
            return new ForceEndTurnResult
                (ForceEndTurnResult.FORCE_ENDTURN_RSRC_DISCARD, discards, true);
        }
    }
     ...
     */
    public static void discardPickRandom(ResourceSet fromHand, int numDiscards, ResourceSet discards, Random rand)
    {
        ...
        for (int rsrcType = ResourceConstants.CLAY;
                rsrcType <= ResourceConstants.WOOD; rsrcType++)
        {
     ...
     *                               or if pn's {@link Player#getNeedToDiscard()} is false,
     *                               or if pn == currentPlayer.
     */
    public ResourceSet playerDiscardRandom(int pn)
        throws IllegalStateException
    {
        ...
        ForceEndTurnResult rs = forceEndTurnChkDiscards(pn);
     ...
     */
    public ResourceSet getResourcesGainedFromRoll(Player player, int roll)
    {
        ResourceSet resources = new ResourceSet();
        ...
        {
            Settlement se = (Settlement) sEnum.nextElement();
        ...
        {
            City ci = (City) cEnum.nextElement();
            ...
            while (hexes.hasMoreElements())
            {
                ...
                if ((board.getNumberOnHexFromCoord(hexCoord) == roll) && (hexCoord != robberHex))
                {
                    ...
                    {
                    case Board.CLAY_HEX:
                        resources.add(2, ResourceConstants.CLAY);

                        break;

                    case Board.ORE_HEX:
                        resources.add(2, ResourceConstants.ORE);

                        break;

                    case Board.SHEEP_HEX:
                        resources.add(2, ResourceConstants.SHEEP);

                        break;

                    case Board.WHEAT_HEX:
                        resources.add(2, ResourceConstants.WHEAT);

                        break;

                    case Board.WOOD_HEX:
                        resources.add(2, ResourceConstants.WOOD);
    ...
    public boolean canDiscard(int pn, ResourceSet rs)
    {
        ...
        ResourceSet resources = players[pn].getResources();
     ...
     * Must not be a desert if {@link GameOption game option} RD is set to true
     ...
     */
    public MoveRobberResult moveRobber(int pn, int co)
    {
        MoveRobberResult result = new MoveRobberResult();
    ...
    public Vector getPlayersOnHex(int hex)
    {
        ...
        for (int i = 0; i < maxPlayers; i++)
        {
            ...
            {
                Settlement se = (Settlement) seEnum.nextElement();
            ...
            if (!touching)
            {
                ...
                {
                    City ci = (City) ciEnum.nextElement();
     ...
     * @return the type of resource that was stolen, as in {@link ResourceConstants}
     */
    ...
    public boolean canMakeTrade(int offering, int accepting)
    {
        ...
        Player offeringPlayer = players[offering];
        Player acceptingPlayer = players[accepting];
        TradeOffer offer = offeringPlayer.getCurrentOffer();
     ...
     * @param offering  the number of the player making the offer
     * @param accepting the number of the player accepting the offer
     * @see #makeBankTrade(ResourceSet, ResourceSet)
     */
    public void makeTrade(int offering, int accepting)
    {
        ...
        ResourceSet offeringPlayerResources = players[offering].getResources();
        ResourceSet acceptingPlayerResources = players[accepting].getResources();
        TradeOffer offer = players[offering].getCurrentOffer();
        ...
        case 3:
                    ...
                    i <= ResourceConstants.WOOD; i++)
            {
                if ((give.getAmount(i) % 3) == 0)
                {
                     ...
                     */
                    if (!(players[currentPlayerNumber].getPortFlag(Board.MISC_PORT)))
                    {
                        return false;
                    }
                }
    ...
    {
        ResourceSet playerResources = players[currentPlayerNumber].getResources();
    ...
    public boolean couldBuildRoad(int pn)
    {
        ...
        return ((resources.getAmount(ResourceConstants.CLAY) >= 1) && (resources.getAmount(ResourceConstants.WOOD) >= 1) && (players[pn].getNumPieces(PlayingPiece.ROAD) >= 1) && (players[pn].hasPotentialRoad()));
    }
    ...
    public boolean couldBuildSettlement(int pn)
    {
        ...
        return ((resources.getAmount(ResourceConstants.CLAY) >= 1) && (resources.getAmount(ResourceConstants.SHEEP) >= 1) && (resources.getAmount(ResourceConstants.WHEAT) >= 1) && (resources.getAmount(ResourceConstants.WOOD) >= 1) && (players[pn].getNumPieces(PlayingPiece.SETTLEMENT) >= 1) && (players[pn].hasPotentialSettlement()));
    }
    ...
    public boolean couldBuildCity(int pn)
    {
        ...
        return ((resources.getAmount(ResourceConstants.ORE) >= 3) && (resources.getAmount(ResourceConstants.WHEAT) >= 2) && (players[pn].getNumPieces(PlayingPiece.CITY) >= 1) && (players[pn].hasPotentialCity()));
    }
    ...
    public boolean couldBuyDevCard(int pn)
    {
        ...
        return ((resources.getAmount(ResourceConstants.SHEEP) >= 1) && (resources.getAmount(ResourceConstants.ORE) >= 1) && (resources.getAmount(ResourceConstants.WHEAT) >= 1) && (numDevCards > 0));
    }
    ...
    {
        ResourceSet resources = players[pn].getResources();
        resources.subtract(1, ResourceConstants.CLAY);
        resources.subtract(1, ResourceConstants.WOOD);
    ...
    public void buySettlement(int pn)
    {
        ResourceSet resources = players[pn].getResources();
        resources.subtract(1, ResourceConstants.CLAY);
        resources.subtract(1, ResourceConstants.SHEEP);
        resources.subtract(1, ResourceConstants.WHEAT);
        resources.subtract(1, ResourceConstants.WOOD);
        oldGameState = gameState;  // PLAY1 or SPECIAL_BUILDING
    ...
    {
        ResourceSet resources = players[pn].getResources();
        resources.subtract(3, ResourceConstants.ORE);
        resources.subtract(2, ResourceConstants.WHEAT);
        oldGameState = gameState;  // PLAY1 or SPECIAL_BUILDING
    ...
    {
        ResourceSet resources = players[pn].getResources();
        resources.add(3, ResourceConstants.ORE);
        resources.add(2, ResourceConstants.WHEAT);
        if (oldGameState != SPECIAL_BUILDING)
            gameState = PLAY1;
        else
    ...
    public int buyDevCard()
    {
        ...
        ResourceSet resources = players[currentPlayerNumber].getResources();
        resources.subtract(1, ResourceConstants.ORE);
        resources.subtract(1, ResourceConstants.SHEEP);
        resources.subtract(1, ResourceConstants.WHEAT);
        players[currentPlayerNumber].getDevCards().add(1, DevCardSet.NEW, card);
        checkForWinner();
    ...
    public boolean canPlayKnight(int pn)
    {
        ...
        if (players[pn].getDevCards().getAmount(DevCardSet.OLD, DevCardConstants.KNIGHT) == 0)
        {
    ...
    public boolean canPlayRoadBuilding(int pn)
    {
        ...
        if (players[pn].getDevCards().getAmount(DevCardSet.OLD, DevCardConstants.ROADS) == 0)
        {
            D.ebugPrintln("cannot play RoadBuilding: already played something.");
            return false;
        }

        if (players[pn].getNumPieces(PlayingPiece.ROAD) < 1)
        {
    ...
    public boolean canPlayDiscovery(int pn)
    {
        ...
        if (players[pn].getDevCards().getAmount(DevCardSet.OLD, DevCardConstants.DISC) == 0)
        {
    ...
    public boolean canPlayMonopoly(int pn)
    {
        ...
        if (players[pn].getDevCards().getAmount(DevCardSet.OLD, DevCardConstants.MONO) == 0)
        {
    ...
    public void playKnight()
    {
        ...
        players[currentPlayerNumber].getDevCards().subtract(1, DevCardSet.OLD, DevCardConstants.KNIGHT);
    ...
    {
        players[currentPlayerNumber].setPlayedDevCard(true);
        players[currentPlayerNumber].getDevCards().subtract(1, DevCardSet.OLD, DevCardConstants.ROADS);
        oldGameState = gameState;
        if (players[currentPlayerNumber].getNumPieces(PlayingPiece.ROAD) > 1)
        {
    ...
    {
        players[currentPlayerNumber].setPlayedDevCard(true);
        players[currentPlayerNumber].getDevCards().subtract(1, DevCardSet.OLD, DevCardConstants.DISC);
        oldGameState = gameState;
    ...
    {
        players[currentPlayerNumber].setPlayedDevCard(true);
        players[currentPlayerNumber].getDevCards().subtract(1, DevCardSet.OLD, DevCardConstants.MONO);
        oldGameState = gameState;
     ...
     */
    public void doDiscoveryAction(ResourceSet pick)
    {
        for (int i = ResourceConstants.CLAY; i <= ResourceConstants.WOOD;
                i++)
        {
     ...
     * placed piece, by calling their {@link Player#calcLongestRoad2()}.
     * Assumes all other players' longest road has been updated already.
     * All players' {@link Player#getLongestRoadLength()} is called here.
     *<P>
     ...
     * by the {@link GameBoardReset} constructor.
     ...
     */
    public Game resetAsCopy()
    {
        Game cp = new Game(name, active, GameOption.cloneOptions(opts));
    ...
    public boolean canBuyOrAskSpecialBuild(final int pn)
    {
        return
          ((pn == currentPlayerNumber)
            && ((gameState == Game.PLAY1) || (gameState == Game.SPECIAL_BUILDING)))
     ...
     *            or {@link Player#hasAskedSpecialBuild() pn.hasAskedSpecialBuild()}
     *            or {@link Player#hasSpecialBuilt() pn.hasSpecialBuilt()} is true,
     ...
     * You can clear the player's flag via {@link Player#setAskedSpecialBuild(boolean)},
