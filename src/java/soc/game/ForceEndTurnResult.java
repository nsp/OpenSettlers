...
/**
 * This class holds the results of a call to {@link Game#forceEndTurn()}.
 ...
 */
public class ForceEndTurnResult
{
     ...
     * from constants such as {@link DevCardConstants#DISC}.
     */
    private int devCardType;

    /**
     * If true, game's {@link Game#getFirstPlayer()} was changed.
     * @since 1.1.09
     */
     ...
     * Calling {@link Game#setFirstPlayer(int)} will also calculate
    ...
    /**
     * {@link Game#forceEndTurn()} return values. FORCE_ENDTURN_MIN is the lowest valid value.
     */
    public static final int FORCE_ENDTURN_MIN               = 1;  // Lowest possible

    /** Since state is already {@link Game#PLAY1}, already OK to end turn. No action was taken by forceEndTurn. */
    public static final int FORCE_ENDTURN_NONE              = 1;

    /**
     * Skip an initial road or settlement; current player has advanced forward, state changes to {@link Game#START1A}.
    ...
    /**
     * Skip an initial road or settlement; current player has advanced backward, state changes to {@link Game#START2A}.
     * May have changed game's firstPlayer or lastPlayer; check {@link #didUpdateFP()} and {@link #didUpdateLP()}.
     */
    public static final int FORCE_ENDTURN_SKIP_START_ADVBACK = 3;

    /** Skip an initial road or settlement; state changes to {@link Game#PLAY1}, and {@link Game#endTurn()} should be called. */
    ...
    /** Resources have been randomly discarded. Cannot end turn yet; other players must discard. {@link Game#isForcingEndTurn()} is set. */
    ...
    /** Highest valid FORCE_ENDTURN_ value for {@link Game#forceEndTurn()} */
    public static final int FORCE_ENDTURN_MAX               = 9;  // Highest possible

    /**
     * Creates a new ForceEndTurnResult object, no resources gained/lost.
     *
     ...
     */
    public ForceEndTurnResult(int res)
    {
        this(res, null, false, false, false);
    }

    /**
     * Creates a new ForceEndTurnResult object, from start states, possibly changing the game's firstplayer or lastplayer.
     *
     * @param res Result type, from constants in this class
     *            ({@link #FORCE_ENDTURN_UNPLACE_ROBBER}, etc.)
     * @param updateFirstPlayer Was {@link Game#getFirstPlayer()} changed?
     * @param updateLastPlayer  Was game's lastPlayer changed?
     * @throws IllegalArgumentException If res is not in the range
     *            {@link #FORCE_ENDTURN_MIN} to {@link #FORCE_ENDTURN_MAX}.
     * @since 1.1.09
     */
    public ForceEndTurnResult(int res, final boolean updateFirstPlayer, final boolean updateLastPlayer)
    {
        this(res, null, false, updateFirstPlayer, updateLastPlayer);
    }

    /**
     * Creates a new ForceEndTurnResult object, with a development card regained.
     *
     * @param res Result type, from constants in this class
     *            ({@link #FORCE_ENDTURN_UNPLACE_ROBBER}, etc.)
     * @param dtype Development card type, like {@link DevCardConstants#DISC}, or -1 for none.
     * @throws IllegalArgumentException If res is not in the range
     *            {@link #FORCE_ENDTURN_MIN} to {@link #FORCE_ENDTURN_MAX},
     *            or if dtype is not -1 and not in the range
     *            {@link DevCardConstants#MIN} to {@link DevCardConstants#MAX_KNOWN}.
     */
    public ForceEndTurnResult(int res, int dtype)
    {
        this(res);
        if ( ((dtype < DevCardConstants.MIN) || (dtype > DevCardConstants.MAX_KNOWN))
            && (dtype != -1) )
    ...
    /**
     * Creates a new ForceEndTurnResult object, with resources gained.
     *
     * @param res Result type, from constants in this class
     *            ({@link #FORCE_ENDTURN_UNPLACE_ROBBER}, etc.)
     ...
     *            {@link #FORCE_ENDTURN_MIN} to {@link #FORCE_ENDTURN_MAX}.
     */
    public ForceEndTurnResult(int res, ResourceSet gained)
    {
        this(res, gained, false, false, false);
    }

    /**
     * Creates a new ForceEndTurnResult object, with resources gained/lost.
     *
     * @param res Result type, from constants in this class
     *            ({@link #FORCE_ENDTURN_UNPLACE_ROBBER}, etc.)
     ...
     *            {@link #FORCE_ENDTURN_MIN} to {@link #FORCE_ENDTURN_MAX}.
     */
    public ForceEndTurnResult(int res, ResourceSet gainedLost, boolean isLoss)
    {
        this(res, gainedLost, isLoss, false, false);
    }

    /**
     * Creates a new ForceEndTurnResult object.
     *
     * @param res Result type, from constants in this class
     *            ({@link #FORCE_ENDTURN_UNPLACE_ROBBER}, etc.)
     * @param gainedLost Resources gained (returned to cancel piece
     ...
     *            {@link #FORCE_ENDTURN_MIN} to {@link #FORCE_ENDTURN_MAX}.
     */
    private ForceEndTurnResult(int res, ResourceSet gainedLost, boolean isLoss,
     ...
     */
    public ResourceSet getResourcesGainedLost()
    {
        return gainLoss;
    }
    ...
    /**
     * Did the game's {@link Game#getFirstPlayer()} change?
     * @since 1.1.09
     */
     ...
     *         like {@link DevCardConstants#DISC}.
     */
