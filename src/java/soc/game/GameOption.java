 ...
 * game option settings; only a very few places use GameOption
 ...
 * {@link Message#isSingleLineAndSafe(String)} within constructors and setters.
 *<P>
 ...
 * @since 1.1.07
 */
public class GameOption implements Cloneable, Comparable
{
    /**
     * Set of "known options".
     * allOptions must never be null, because other places assume it is filled.
     */
    private static Hashtable<String, GameOption> allOptions = initAllOptions();
     ...
     *<LI> Within {@link Game}, don't add any object fields due to the new option;
     *   instead call {@link Game#isGameOptionDefined(String)},
     *   {@link Game#getGameOptionIntValue(String)}, etc.
     ...
     *   <LI> {@link soc.client.PlayerClient} is the graphical client
     *   <LI> {@link soc.robot.RobotClient} and {@link soc.robot.RobotBrain#run()}
     *           together handle the robot client messages
     *   <LI> {@link soc.client.DisplaylessPlayerClient} is the foundation for the robot client,
     ...
     *   {@link soc.util.RobotParameters#copyIfOptionChanged(Hashtable)}
     ...
     */
    public static Hashtable<String, GameOption> initAllOptions()
    {
        Hashtable<String, GameOption> opt = new Hashtable<String, GameOption>();

        opt.put("PL", new GameOption
                ("PL", -1, 1108, 4, 2, 6, "Maximum # players"));
        opt.put("RD", new GameOption
                ("RD", -1, 1107, false, false, "Robber can't return to the desert"));
        opt.put("N7", new GameOption
                ("N7", -1, 1107, false, 7, 1, 999, false, "Roll no 7s during first # rounds"));
        opt.put("BC", new GameOption
                ("BC", -1, 1107, true, 4, 3, 9, false, "Break up clumps of # or more same-type hexes/ports"));
        opt.put("NT", new GameOption
     ...
     * characters {@link Message#sep_char} or {@link Message#sep2_char}.
     ...
     * @throws IllegalArgumentException if key length is > 3 or not alphanumeric,
     *        or if desc contains {@link Message#sep_char} or {@link Message#sep2_char},
     *        or if minVers or lastModVers is under 1000 but not -1
     */
    public GameOption(String key, int minVers, int lastModVers,
     ...
     *        or if minVers or lastModVers is under 1000 but not -1
     */
    public GameOption(String key, int minVers, int lastModVers, boolean defaultBoolValue, int defaultIntValue,
     ...
     *        or if minVers or lastModVers is under 1000 but not -1
     */
    public GameOption(String key, int minVers, int lastModVers, boolean defaultBoolValue,
     ...
     * @param desc Descriptive brief text, to appear in the options dialog; should
     *             contain a placeholder character '#' where the int value goes.
     *             Desc must not contain {@link Message#sep_char} or {@link Message#sep2_char},
     *             and must evaluate true from {@link Message#isSingleLineAndSafe(String)}.
     * @throws IllegalArgumentException if defaultIntValue < minValue or is > maxValue,
     *        or if key length is > 3 or not alphanumeric,
     *        or if desc contains {@link Message#sep_char} or {@link Message#sep2_char},
     *        or if minVers or lastModVers is under 1000 but not -1
     */
    protected GameOption(int otype, String key, int minVers, int lastModVers,
	...
	// validate & set option properties:
        ...
        if (! Message.isSingleLineAndSafe(desc))
	...
	optDesc = desc;
        ...
        if (enumVals != null)
        {
            for (int i = enumVals.length - 1; i>=0; --i)
                if (! Message.isSingleLineAndSafe(enumVals[i]))
     ...
     */
    protected GameOption(GameOption enumOpt, String[] keptEnumVals)
     ...
     * @since 1.1.08
     */
    protected GameOption(GameOption intOpt, final int maxIntValue)
    {
     ...
     * Will not contain newlines or otherwise fail {@link Message#isSingleLineAndSafe(String)}.
     */
     ...
     *          Must not contain {@link Message#sep_char} or {@link Message#sep2_char} ('|' or ',').
     * @throws IllegalArgumentException if v contains characters reserved for
     *          message handling: {@link Message#sep} or 
     *          {@link Message#sep2} ('|' or ','), or is
     *          multi-line or otherwise fails {@link Message#isSingleLineAndSafe(String)}.
     */
	...
	throws IllegalArgumentException
    {
        if (v != null)
        {
            ...
            if (vl == 0)
            {
                ...
                if (! Message.isSingleLineAndSafe(v))
     ...
     * @see #addKnownOption(GameOption)
     */
    public static Hashtable<String, GameOption> getAllKnownOptions()
    {
     ...
     */
    public static boolean addKnownOption(GameOption onew)
    {
     ...
     */
    public static void setKnownOptionCurrentValue(GameOption ocurr)
        throws IllegalArgumentException
    {
        final String oKey = ocurr.optKey;
        GameOption oKnown = allOptions.get(oKey);
        if (oKnown == null)
            return;
     ...
     */
    public static Hashtable<String, GameOption> cloneOptions(Hashtable<String, GameOption> opts)
    {
	if (opts == null)
	    return null;

	Hashtable<String, GameOption> opts2 = new Hashtable<String, GameOption>();
	for (Enumeration<String> e = opts.keys(); e.hasMoreElements(); )
	{
	    GameOption op = opts.get(e.nextElement());
	    try
	    {
	        opts2.put(op.optKey, (GameOption) op.clone());
     ...
     */
    public static GameOption getOption(String key)
    {
     ...
     * @param opts hashtable of GameOption
     * @return vector(GameOption) of unknown options, or null if all are known
     */
    public static Vector<String> findUnknowns(Hashtable opts)
    {
        Vector<String> unknowns = null;
        for (Enumeration e = opts.keys(); e.hasMoreElements(); )
        {
            GameOption op = (GameOption) opts.get((String) e.nextElement());
     ...
     */
    public static String packOptionsToString(Hashtable<String, GameOption> ohash, boolean hideEmptyStringOpts)
	...
	for (Enumeration<String> e = ohash.keys(); e.hasMoreElements(); )
	{
	    GameOption op = ohash.get(e.nextElement());
     ...
     * @see #parseOptionNameValue(String, boolean)
     */
    public static Hashtable<String, GameOption> parseOptionsToHash(String ostr)
    {
	if ((ostr == null) || ostr.equals("-"))
	    return null;

	Hashtable<String, GameOption> ohash = new Hashtable<String, GameOption>();

	StringTokenizer st = new StringTokenizer(ostr, Message.sep2);
	String nvpair;
	while (st.hasMoreTokens())
	{
	    nvpair = st.nextToken();  // skips any leading commas or doubled commas
            GameOption copyOpt = parseOptionNameValue(nvpair, false);
            if (copyOpt == null)
     ...
     * @see #packValue(StringBuffer)
     */
    public static GameOption parseOptionNameValue(final String nvpair, final boolean forceNameUpcase)
    {
        ...
        GameOption knownOpt = allOptions.get(optkey);
        GameOption copyOpt;
        if (knownOpt == null)
        {
            copyOpt = new GameOption(optkey);  // OTYPE_UNKNOWN
        }
        else
        {
            if ((optval.length() == 0)
            ...
            try
            {
                copyOpt = (GameOption) knownOpt.clone();
            } catch (CloneNotSupportedException ce)
            {
                return null;
            }
     ...
     * @throws ClassCastException if values contain a non-{@link GameOption}
     * @see #getMinVersion()
     */
	...
	for (Enumeration e = opts.keys(); e.hasMoreElements(); )
	{
	    GameOption op = (GameOption) opts.get(e.nextElement());
     ...
     * @param opts  Set of {@link GameOption}s to check current values;
     *              if null, use the "known option" set
     * @return Vector of the newer {@link GameOption}s, or null
     *     if all are known and unchanged since <tt>vers</tt>.
     */
    public static Vector<GameOption> optionsNewerThanVersion(final int vers, final boolean checkValues, final boolean trimEnums, Hashtable<String, GameOption> opts)
    {
        if (opts == null)
            opts = allOptions;
        Vector<GameOption> uopt = null;  // add problems to uopt
        
        for (Enumeration<GameOption> e = opts.elements(); e.hasMoreElements(); )
        {
            GameOption opt = e.nextElement();
     ...
     */
    public static GameOption trimEnumForVersion(GameOption opt, final int vers)
    {
        final int ev = getMaxEnumValueForVersion(opt.optKey, vers);
        ...
        return new GameOption(opt, evkeep);  // Copy option and restrict enum values
    }
     ...
     * @throws IllegalArgumentException if newOpts contains a non-GameOption
     */
    public static boolean adjustOptionsToKnown(Hashtable newOpts, Hashtable<String, GameOption> knownOpts)
        throws IllegalArgumentException
    {
        if (knownOpts == null)
     ...
     * @param other A GameOption to compare, or another object;  if other isn't a
     *              gameoption, the {@link #hashCode()}s are compared.
     */
    public int compareTo(Object other)
    {
        if (other instanceof GameOption)
        {
            GameOption oopt = (GameOption) other;
