 ...
 */
class QuitConfirmDialog extends AskDialog
{
    private static final long serialVersionUID = -7909114816498528932L;

    /**
     * Creates and shows a new QuitConfirmDialog.
     ...
     */
    public static void createAndShow(PlayerClient cli, PlayerInterface gamePI)
        throws IllegalArgumentException
    {
        if ((cli == null) || (gamePI == null))
            throw new IllegalArgumentException("no nulls");
        Game ga = gamePI.getGame();
        boolean gaOver = (ga.getGameState() >= Game.OVER);

        QuitConfirmDialog qcd = new QuitConfirmDialog(cli, gamePI, gaOver);
        qcd.setVisible(true);      
    }
    

    /**
     * Creates a new QuitConfirmDialog.
     *
     * @param cli      Player client interface
     * @param gamePI   Current game's player interface
     * @param gameIsOver The game is over - "Quit" button should be default (if not over, Continue is default)
     */
    protected QuitConfirmDialog(PlayerClient cli, PlayerInterface gamePI, boolean gameIsOver)
    {
        super(cli, gamePI, "Really quit game "
            ...
            (gameIsOver
                ? "Don't quit"
                : "Continue playing"),
            ((gamePI.getGame().getGameState() != Game.NEW)
                ? "Reset board"
                : null),
            (gameIsOver ? 1 : 2));
    }
