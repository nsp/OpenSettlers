 ...
 * Contains {@link BoardPanel board}, client's and other players' {@link HandPanel hands},
 * chat interface, game message window, and the {@link BuildingPanel building/buying panel}.
 *<P>
 * Players' {@link HandPanel hands} start with player 0 at top-left, and go clockwise;
 ...
 *<P>
 * A separate {@link PlayerClient} window holds the list of current games and channels.
 *
 * @author Robert S. Thomas
 */
public class PlayerInterface extends Frame implements ActionListener, MouseListener
{
     ...
     * turn begins (state changes from {@link Game#START2B}
     * to {@link Game#PLAY}).
    ...
    public PlayerInterface(String title, PlayerClient cl, Game ga)
    {
        ...
        if (is6player)
            setSize((2*HandPanel.WIDTH_MIN) + 16 + boardPanel.getMinimumSize().width, piHeight);
        else
...
//        textInput.addKeyListener(new InputKeyListener());
    ...
    public void updateLongestLargest
        (boolean isRoadNotArmy, Player oldp, Player newp)
    {
        // Update handpanels
     ...
     * if game parameter "PL" is less than {@link Game#maxPlayers}.
     * Also, if show, and gamestate is {@link Game#NEW}, check for game-is-full,
    ...
    private void updatePlayerLimitDisplay(final boolean show, final int playerLeaving)
    {
        ...
        boolean noTextOverlay =  ((! show) ||
            ((gstate >= Game.START1A) && clientSatAlready));
        if (noTextOverlay)
        {
    ...
    /** Update the client player's HandPanel interface, for joining
     *  or leaving a game.
     *  
     *  Set by HandPanel's removePlayer() and addPlayer() methods.
     *  
     * @param h  The HandPanel for us, or null if none (leaving).
     */ 
    ...
    /**
     * Game play is starting (leaving state {@link Game#NEW}).
    ...
    public void updateAtOver(int[] finalScores)
    {
        ...
        for (int i = 0; i < finalScores.length; ++i)
            hands[i].updateValue(HandPanel.VICTORYPOINTS);  // Also disables buttons, etc.
     ...
     * Please call after {@link Game#setGameState(int)}.
    ...
    /**
     * Gamestate just became {@link Game#WAITING_FOR_DISCARDS}.
     ...
     * @see soc.server.SOCServer#resetBoardAndNotify(String, int)
     */
    public void resetBoard(Game newGame, int rejoinPlayerNumber, int requesterNumber)
    {
        if (clientHand == null)
            return;
     ...
     * {@link #invalidate()} and call this again, because {@link HandPanel} sizes will change.
    ...
    public void doLayout()
    {
         ...
         *
        int bw = BoardPanel.getPanelX();
        int bh = BoardPanel.getPanelY();
        ...
        int bw = (dim.width - 16 - (2*HandPanel.WIDTH_MIN));  // As wide as possible
    ...
    protected static class ResetBoardVoteDialog extends AskDialog implements Runnable
    {
         ...
         */
        protected ResetBoardVoteDialog(PlayerClient cli, PlayerInterface gamePI, String requester, boolean gameIsOver)
        {
