 ...
 * Sits within a game's {@link PlayerInterface} frame.
 */
public class BuildingPanel extends Panel implements ActionListener
{
     ...
     */
    public BuildingPanel(PlayerInterface pi)
    {
        super();
        setLayout(null);
     ...
     * @param doNotClearPopup Do not call {@link BoardPanel#popupClearBuildRequest()}
     */
    public void clickBuildingButton(Game game, PlayerClient client, String target, boolean doNotClearPopup)
    {
        if (! doNotClearPopup)
        ...
        final boolean stateBuyOK =        // same as in updateButtonStatus.
            (isCurrent)
            ? ((gstate == Game.PLAY1) || (gstate == Game.SPECIAL_BUILDING))
            : canAskSBP;
        ...
        if (target == ROAD)
        {
            ...
            {
                if (stateBuyOK)
                    sendBuildRequest = PlayingPiece.ROAD;
                else if (canAskSBP)
                    sendBuildRequest = -1;
            }
            else if (roadBut.getLabel().equals("Cancel"))
            {
                client.cancelBuildRequest(game, PlayingPiece.ROAD);
            }
        }
        else if (target == STLMT)
        {
            ...
            {
                if (stateBuyOK)
                    sendBuildRequest = PlayingPiece.SETTLEMENT;
                else if (canAskSBP)
                    sendBuildRequest = -1;
            }
            else if (settlementBut.getLabel().equals("Cancel"))
            {
                client.cancelBuildRequest(game, PlayingPiece.SETTLEMENT);
            }
        }
        else if (target == CITY)
        {
            ...
            {
                if (stateBuyOK)
                    sendBuildRequest = PlayingPiece.CITY;
                else if (canAskSBP)
                    sendBuildRequest = -1;
            }
            else if (cityBut.getLabel().equals("Cancel"))
            {
                client.cancelBuildRequest(game, PlayingPiece.CITY);
            }
        }
        else if (target == CARD)
        {
    ...
    public void updateButtonStatus()
    {
        ...
        if (player != null)
        {
            ...
            if (isCurrent && (gstate == Game.PLACING_ROAD))
            {
            ...
            if (isCurrent &&
                ((gstate == Game.PLACING_SETTLEMENT) || (gstate == Game.START1B)
                 || (gstate == Game.START2B)))
            {
            ...
            if (isCurrent && (gstate == Game.PLACING_CITY))
            {
