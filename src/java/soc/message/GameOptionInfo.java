...
/**
 * Information on one available {@link GameOption game option}.
 * Reply from server to a client's {@link GameOptionGetInfos GAMEOPTIONGETINFOS} message.
 ...
 * the server will respond with {@link GameOption#OTYPE_UNKNOWN}.
 *<P>
 ...
 *<P>
 * Introduced in 1.1.07; check client version against {@link NewGameWithOptions#VERSION_FOR_NEWGAMEWITHOPTIONS}
 ...
 * @since 1.1.07
 */
public class GameOptionInfo extends MessageTemplateMs
{
     ...
     */
    public static final GameOptionInfo OPTINFO_NO_MORE_OPTS
        = new GameOptionInfo(new GameOption("-"));
    ...
    protected GameOption opt = null;

    /** Constructor for server to tell client about a game option */
    public GameOptionInfo(GameOption op)
    {
        // OTYPE_*
        super(GAMEOPTIONINFO, null,
            new String[12 + ( ((op.optType != GameOption.OTYPE_ENUM)
                               && (op.optType != GameOption.OTYPE_ENUMBOOL))
                    ? 0
                    : op.maxIntValue ) ]);
        opt = op;
        pa[0] = op.optKey;
        ...
        if ((op.optType == GameOption.OTYPE_STR) || (op.optType == GameOption.OTYPE_STRHIDE))
        {
        ...
        if ((op.optType == GameOption.OTYPE_ENUM) || (op.optType == GameOption.OTYPE_ENUMBOOL))
     ...
     * If opt type number is unknown locally, will change to {@link GameOption#OTYPE_UNKNOWN}.
     *
     ...
     *
     * @throws IllegalArgumentException if pa.length < 11, or type is not a valid {@link GameOption#optType};
     *      if type isn't {@link GameOption#OTYPE_ENUM OTYPE_ENUM} or ENUMBOOL, pa.length must == 12 (or 11 for OTYPE_UNKNOWN).
     * @throws NumberFormatException    if pa integer-field contents are incorrectly formatted.
     */
    protected GameOptionInfo(String[] pa)
	...
	int otyp = Integer.parseInt(pa[1]);
        if ((otyp < GameOption.OTYPE_MIN) || (otyp > GameOption.OTYPE_MAX))
            otyp = GameOption.OTYPE_UNKNOWN;
	...
	final int ival_cur;
	String sval_cur;
	if ((otyp == GameOption.OTYPE_STR) || (otyp == GameOption.OTYPE_STRHIDE))
	{
	    ival_cur = 0;
	    sval_cur = pa[9];
        ...
        if ((pa.length != 11) && (pa.length != 12)
              && (otyp != GameOption.OTYPE_ENUM)
              && (otyp != GameOption.OTYPE_ENUMBOOL))
	    throw new IllegalArgumentException("pa.length");

	switch (otyp)  // OTYPE_*
	{
	case GameOption.OTYPE_UNKNOWN:
	    opt = new GameOption(pa[0]);
	    break;

	case GameOption.OTYPE_BOOL:
	    opt = new GameOption(pa[0], oversmin, oversmod, bval_def, skip_def, pa[11]);
	    opt.setBoolValue(bval_cur);
	    break;

	case GameOption.OTYPE_INT:
	    opt = new GameOption(pa[0], oversmin, oversmod, ival_def, ival_min, ival_max, pa[11]);  // skip_def ignored
	    opt.setIntValue(ival_cur);
	    break;

	case GameOption.OTYPE_INTBOOL:
	    opt = new GameOption(pa[0], oversmin, oversmod, bval_def, ival_def, ival_min, ival_max, skip_def, pa[11]);
	    opt.setBoolValue(bval_cur);
	    opt.setIntValue(ival_cur);
	    break;
	...
	case GameOption.OTYPE_ENUM:
		...
		System.arraycopy(pa, 12, choices, 0, ival_max);
	        opt = new GameOption(pa[0], oversmin, oversmod, ival_def, choices, pa[11]);  // skip_def ignored
	        opt.setIntValue(ival_cur);
            }
	    break;
        ...
        case GameOption.OTYPE_ENUMBOOL:
            {
                String[] choices = new String[ival_max];
                System.arraycopy(pa, 12, choices, 0, ival_max);
                opt = new GameOption(pa[0], oversmin, oversmod, bval_def, ival_def, choices, skip_def, pa[11]);
                opt.setBoolValue(bval_cur);
                opt.setIntValue(ival_cur);
            }
            break;
	...
	case GameOption.OTYPE_STRHIDE:
	    opt = new GameOption
		(pa[0], oversmin, oversmod, ival_max, (otyp == GameOption.OTYPE_STRHIDE), skip_def, pa[11]);
     ...
     */
    public GameOption getOptionInfo()
    {
        return opt;
    }

    /**
     * Parse the command String array into a GameOptionInfo message. <pre>
     * pa[0] = key (name of the {@link GameOption option})
     * pa[1] = type
     * pa[2] = minVersion
     * pa[3] = lastModVersion
     * pa[4] = defaultBoolValue ('t' or 'f')
     ...
     */
    public static GameOptionInfo parseDataStr(String[] pa)
    {
        if ((pa == null) || (pa.length < 11))
            return null;
        try
        {
            return new GameOptionInfo(pa);
        } catch (Throwable e)
        {
            return null;
        }
