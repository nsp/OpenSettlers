 ...
 * original game, {@link soc.game.Board#BOARD_ENCODING_ORIGINAL}. 
 * As of version 1.1.08, there is a newer board layout for
 * game expansions.  See the new message type {@link BoardLayout2 BOARDLAYOUT2}.
 *<P>
 * Unlike {@link BoardLayout2}, the dice numbers are mapped before sending
 * over the network, and unmapped when received.  This is because of a change in
 * {@link soc.game.Board#getNumberLayout()} in 1.1.08, the version which
 * introduced <tt>BoardLayout2</tt>.  Older clients/servers will still need the
 * mapping done, so it's now done here instead of in Board.
 *
 * @author Robert S. Thomas
 */
public class BoardLayout extends Message
{
    private static final long serialVersionUID = -3749424346571547319L;

    /**
     * Map of dice rolls to values in {@link #numberLayout}. Formerly in Board.
     * @since 1.1.08
     */
     ...
     */
    public BoardLayout(String ga, int[] hl, int[] nl, int rh)
    {
     ...
     * @since 1.1.08
     */
    private BoardLayout(String ga, int[] hl, int[] nl, int rh, boolean alreadyMappedNL)
    {
    ...
    public int[] getNumberLayout()
    {
        ...
        for (int i = nl.length - 1; i >= 0; --i)
        {
            ...
            if (n != -1)
                ...
                n = 0;  // 0, not -1, in 1.1.07 Board.getNumberOnHexFromNumber
            nl[i] = n;
        }
        return nl;
     ...
     */
    public static BoardLayout parseDataStr(String s)
    {
        String ga; // game name
        ...
        return new BoardLayout(ga, hl, nl, rh, true);
    }

    /**
     * Render the BoardLayout in human-readable form.
    ...
    {
        StringBuffer sb = new StringBuffer("BoardLayout:game=");
        sb.append(game);
        sb.append("|hexLayout=");
